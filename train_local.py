"""
Ovarian Cancer CT Classification - Residual 3D CNN
"""
...
class CTDataset(Dataset):
    def __init__(self, flist, labs, aug=False):
        self.flist, self.labs = flist, labs; self.tf = make_transforms(aug)
...
class EMA:
    def __init__(self, m, d=0.999): self.m,self.d=m,d; self.sh={n:p.data.clone() for n,p in m.named_parameters() if p.requires_grad}; self.bk={}
    def upd(self):
...
class Trainer:
    def __init__(self, mdl, fdir):
        self.mdl=mdl.to(DEVICE); self.fdir=fdir; os.makedirs(fdir, exist_ok=True)
        ...
        self.scl=GradScaler(); self.ema=EMA(mdl); self.best,self.pat=0,0
    def run(self, dl, train):
        ...
        for x,y in dl:
            ...
            if train:
                ...
                self.scl.scale(loss).backward(); self.scl.unscale_(self.opt)
            ...
            ls+=loss.item(); p=torch.softmax(out.detach(),1)
    ...
    def fit(self, tr, va, ep):
        for e in range(ep):
            tl,ta,_,_,_,_=self.run(tr,True); vl,vacc,vau,_,_,_=self.run(va,False); self.sch.step()
...
print(f"Res3DCNN | {n_params:,} params | {FINAL_SIZE} | Batch={BATCH_SIZE}")
print("="*50)
...
for fold in range(N_FOLDS):
    ...
    tr_dl=DataLoader(CTDataset(tr_f,tr_l,True),BATCH_SIZE,shuffle=True,num_workers=0,pin_memory=True)
    va_dl=DataLoader(CTDataset(va_f,va_l,False),BATCH_SIZE,shuffle=False,num_workers=0,pin_memory=True)
    te_dl=DataLoader(CTDataset(te_f,te_l,False),BATCH_SIZE,shuffle=False,num_workers=0,pin_memory=True)
    t=Trainer(Res3DCNN(),fd); t.fit(tr_dl,va_dl,NUM_EPOCHS); t.load()
